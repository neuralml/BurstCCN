...
class ContinuousBurstPropOutputLayer:
    ...
    def feedback_update(self, target=None, dt=0.1):
        ...
        if target is not None:
            ...
            delta_burst_prob = self.p_baseline * (target - self.event_rate_ma) * (1 - self.event_rate_ma)
    ...
    def weight_update(self, input_event_rate, dt=0.1):
        ...
        self.bias = self.bias + (dt / self.tau_W) * ((self.burst_prob_cache - self.burst_prob_ma_cache) * self.event_rate_cache)
        ...
        self.weight = self.weight + (dt / self.tau_W) * (((self.burst_prob_cache - self.burst_prob_ma_cache) * self.event_rate_cache) * input_event_rate.t())


class ContinuousBurstPropHiddenLayer:
    def __init__(self, in_features, out_features, next_features, device):
        self.in_features = in_features
        self.out_features = out_features
        self.device = device
    ...
    def feedback_update(self, next_layer_burst_rate_cache, dt=0.1):
        self.dendritic_potential = (1. - dt / self.tau_u) * self.dendritic_potential + (dt / self.tau_u) * (self.beta * self.h(self.event_rate_cache) * (self.weight_Y.T.mm(next_layer_burst_rate_cache)))
