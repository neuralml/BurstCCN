...
class BurstPropOutputLayer(nn.Module):
    def __init__(self, in_features, out_features, p_baseline, weight_Y_learning, device):
    ...
    def backward(self, b_input):
        ...
        # if self.prev_b_t is not None:
        #     self.delta = -(0.1 * (self.b_t - self.prev_b_t) + 0.9 * (self.b_t - self.b))
        # else:
        #     self.delta = -0.9 * (self.b_t - self.b)
        ...
        # self.prev_b_t = copy.deepcopy(self.b_t)

        if self.weight_Y_learning:
            delta = -(self.b_t - self.b)
            self.grad_weight_Y = delta.transpose(0, 1).mm(self.input)

        return (self.b).mm(self.weight_Y), (self.b_t).mm(self.weight_Y), self.delta_bp.mm(
            self.weight), self.delta_fa.mm(self.weight_Y)
...
class BurstPropHiddenLayer(nn.Module):
    def __init__(self, in_features, out_features, p_baseline, weight_Y_learning, recurrent_input, weight_r_learning,
                 device):
    ...
    def backward(self, b_input, b_input_t, b_input_bp, b_input_fa):
        ...
        self.delta_fa = b_input_fa * self.e * (1 - self.e)
...
class _ConvNdFA(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, recurrent_input, in_size, device, kernel_size, stride,
        ...
        if self.recurrent_input:
            self.weight_r = torch.Tensor(self.out_size * self.out_size * self.out_channels,
                                         self.out_size * self.out_size * self.out_channels).to(device)
        if bias:
...
class Conv2dBurstPropHiddenLayer(_ConvNdFA):
    def __init__(self, in_channels, out_channels, p_baseline, Y_learning, recurrent_input, weight_r_learning,
                 in_size, device, kernel_size, stride=1,
        ...
        super(Conv2dBurstPropHiddenLayer, self).__init__(
        ...
        if self.Y_learning:
            self.delta_weight_Y = torch.zeros(self.weight_Y.shape).to(device)

            self.grad_weight_Y = torch.zeros(self.weight_Y.shape).to(device)
    ...
    def backward(self, b_input, b_input_t, b_input_bp, b_input_fa):
        ...
        if use_cudnn:
            self.grad_weight = cudnn_convolution.convolution_backward_weight(self.input, self.weight.shape, self.delta,
                                                                             ...
                                                                             self.groups, False, False)
            self.grad_weight_bp = cudnn_convolution.convolution_backward_weight(self.input, self.weight.shape, self.delta_bp,
                                                                             self.stride, self.padding, self.dilation,
                                                                             self.groups, False, False)
            self.grad_weight_fa = cudnn_convolution.convolution_backward_weight(self.input, self.weight.shape, self.delta_fa,
                                                                             self.stride, self.padding, self.dilation,
                                                                             self.groups, False, False)
        else:
        ...
        self.grad_bias_bp = torch.sum(self.delta_bp, dim=[0, 2, 3])
        self.grad_bias_fa = torch.sum(self.delta_fa, dim=[0, 2, 3])

        if self.recurrent_input and self.Y_learning:
            delta = -(self.b_t - self.b)

            if use_cudnn:
                self.grad_weight_Y = cudnn_convolution.convolution_backward_weight(self.input, self.weight_Y.shape,
                                                                                   delta, self.stride, self.padding,
                                                                                   self.dilation, self.groups, False,
                                                                                   False)
            else:
                self.grad_weight_Y = nn.grad.conv2d_weight(self.input, self.weight_Y.shape, delta, self.stride,
                                                           self.padding, self.dilation, self.groups)

        if self.weight_r_learning:
            self.grad_weight_r = -u.view(-1, self.out_size * self.out_size * self.out_channels).transpose(0, 1).mm(
                self.b_pre.view(-1, self.out_size * self.out_size * self.out_channels))

        if use_cudnn:
            return cudnn_convolution.convolution_backward_input(self.input.shape, self.weight_Y, self.b, self.stride,
                                                                self.padding, self.dilation, self.groups, False,
                                                                False), \
                   cudnn_convolution.convolution_backward_input(self.input.shape, self.weight_Y, self.b_t, self.stride,
                                                                self.padding, self.dilation, self.groups, False,
                                                                False), \
                   cudnn_convolution.convolution_backward_input(self.input.shape, self.weight, self.delta_bp,
                                                                self.stride, self.padding, self.dilation,
                                                                self.groups, False, False), \
                   cudnn_convolution.convolution_backward_input(self.input.shape, self.weight_Y, self.delta_fa,
                                                                self.stride, self.padding, self.dilation,
                                                                self.groups, False, False)

        else:
            return nn.grad.conv2d_input(self.input.shape, self.weight_Y, self.b, self.stride, self.padding,
                                        self.dilation, self.groups), \
                   nn.grad.conv2d_input(self.input.shape,
                                        self.weight_Y, self.b_t,
                                        self.stride, self.padding,
                                        self.dilation,
                                        self.groups), \
                   nn.grad.conv2d_input(self.input.shape, self.weight, self.delta_bp, self.stride, self.padding,
                                        self.dilation,
                                        self.groups), \
                   nn.grad.conv2d_input(self.input.shape, self.weight_Y, self.delta_fa, self.stride, self.padding,
                                        self.dilation,
                                        self.groups)
    ...
    def forward_perturb(self, input, unit=None, perturb_layer=True):
        if perturb_layer:
            if unit is None:
                ...
                self.xi = torch.zeros(self.e.shape).to(self.device)
                unravel_unit = unravel_index(unit, self.e.shape[1:])
                self.xi[:, unravel_unit[0], unravel_unit[1], unravel_unit[2]] = self.Xi.sample((self.e.shape[0],)).to(
                    self.device)
